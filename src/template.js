...
if (LEGACY_KEY !== STORE_KEY && !localStorage.getItem(STORE_KEY)) {
...
function trendReadout(trend, ptCount) {
  ...
  if (trend.kind === "smooth") {
    const what = MODE === "dual"
      ? `<b>● ${CONFIG.ySeries[0]}</b>: 실선 · <b>○ ${CONFIG.ySeries[1]}</b>: 점선`
      : "";
    return `추세선: 각 ${CONFIG.xLabel}에서 <b>학급 평균</b>을 부드럽게 이은 곡선${what ? " — " + what : ""}${tail}`;
  }
  const fit = trend.fit;
  if (fit.type === "inverse") {
    return `추세선: <b>${CONFIG.yLabel} ≈ ${fmt(fit.a)} ÷ ${CONFIG.xLabel}${fit.n === 2 ? "²" : ""}</b>${tail}`;
  }
  const bTxt = CONFIG.trendline === "linear" && Math.abs(fit.b) > 1e-9
    ? ` ${fit.b >= 0 ? "+" : "−"} ${fmt(Math.abs(fit.b))}` : "";
  return `추세선: <b>${CONFIG.yLabel} ≈ ${fmt(fit.a)} × ${CONFIG.xLabel}${bTxt}</b>${tail}`;
}
