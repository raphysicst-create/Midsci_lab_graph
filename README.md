...
1. **조 개수 선택·도입** — 실험을 처음 열면 팝업에서 **1~10조**를 선택합니다.
   선택 후 오늘 그래프의 가로축·세로축을 소개합니다 (빈 좌표평면).
2. **조별 입력** — 선택한 조 수만큼 입력 화면과 그래프가 준비됩니다.
   측정값을 넣고 Enter, 숫자가 그래프 위로 날아가 점이 됩니다
...
- 조 개수도 실험별로 저장되며 상단 **조 개수** 버튼에서 바꿀 수 있습니다.
  줄어든 조의 입력값은 보관되고, 다시 늘리면 복원됩니다.
  종합 그래프와 내보내기에는 현재 선택한 조만 포함됩니다.
- **CSV(엑셀)·PNG 내보내기**, 다크 모드, 전체화면을 지원합니다.
- PNG는 실험 제목·조별/계열 범례·그래프·추세선 설명을 포함한 **종합 화면**을
  저장합니다. 현재 테마와 조별 표시·추세선 설정이 반영되며, 어느 씬에서든 저장할 수 있습니다.
