...
한 앱 = 한 실험. 첫 실행 팝업에서 1~10조를 선택한다 (실험별 저장, 상단에서 변경 가능).
씬 흐름: 도입(빈 좌표축) → 선택한 조 수만큼 입력(값 입력 시 표→그래프
점 비행 애니메이션) → 종합(전체 점 + 추세선). 고정된 x값 몇 개에서 각 조가
y값만 입력하는 구조만 지원한다. 입력값은 localStorage에 **slug별** 저장
(제목을 다듬어도 데이터 유지 — 구 제목 키는 최초 1회 자동 이전).
조 수를 줄여도 숨겨진 조의 입력값은 보관하며, 종합·추세선·내보내기에서는 제외한다.
...
| `title` | 앱 제목. 유일해야 함. `\n`을 넣으면 index 카드에서만 줄바꿈으로 표시 |
| `grade` | 중1/중2/… (index.html 분류용) |
| `unit` | 소속 단원 — `N단원_이름` 형식 (예: `3단원_열`). index 카드 표시·단원순 정렬 근거 |
| `summary` | 실험 설명 (위키 근거) — index 카드에 표시. `\n`으로 카드 줄바꿈 위치를 지정 (짧은 2줄 권장) |
...
| `note` | 선택. 교사용 유의점·제안값 근거의 config 기록용 (예: "x값은 교과서에 없는 제안값") — index 카드에는 표시하지 않음 |
...
- `apple-design` · `apple-design-v2` — Apple 디자인 파일럿 실험(2026-07-20~21).
  **폐기 확정** (2026-07-24, 사용자 결정) 후 **2026-08-06 로컬 브랜치 삭제 완료**
  (`e72440a`, `9876d04`). main에 병합되지 않았으며, 다시 시작하지 않는다.
