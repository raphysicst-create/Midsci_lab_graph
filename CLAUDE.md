...
| 필드 | 설명 |
|---|---|
| `slug` | 파일명과 동일, `[a-z0-9_]+`. 위키 전체에서 유일 |
| `title` | 앱 제목 (localStorage 키에도 쓰이므로 바꾸면 저장 데이터가 분리됨). 유일해야 함 |
...
| `trendline` | `"proportional"`(원점 통과) / `"linear"`(절편) / `"inverse"`(반비례 y=a÷xⁿ) / `false`(점만 — 곡선·플래토) |
...
- **y 2계열** `"ySeries": ["물", "식용유"]` — 고정 x에서 조당 y를 2개 입력
  ...
  2번째는 링(○). 두 계열을 하나의 추세선으로 적합하면 안 되므로 **`trendline`은
  반드시 `false`**. 2계열까지만 지원.
